...
function UserDetails() {
    // the user id retrieved from a dynamic link
    ...
    // function to get and store the user from the database in a state variable
    const getUser = async () => {
        try {
    ...
    // function to get and store all the games that the user has made
    ...
    // function to get all comments the user has made on any post on the website
    ...
    // function to convert the createdAt dates made by mongo timestaps to a british date format
    ...
    // loads a spinner until the user is fetched from the database
    ...
    return (
        ...
        <div className="bg-[rgb(8,11,19)] min-h-screen text-[#e2e4ea]">
            ...
            {/* user details using flowbites tabs */}
                ...
                <Tabs theme={{
                    ...
                    <TabItem title={`Games ${userGames.length}`}>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            {
                                // creates an array of games from the array of user games, each game displayed with the specified tailwind css
                                userGames.map((game) => (
                    ...
                    <TabItem title={`Comments ${userComments.length}`}>
                        <div className="flex flex-col gap-3">
                            {
                                // creates an array of comments from the user comments array, displaying each comment with the specified css
                    ...
                    {/* user about */}
                    <TabItem title="About">
                        {/* displays user bio using the bio from the user object */}
                        <div className="bg-[#0d1020] border border-[#1e2236] rounded-xl px-5 py-4">
                            <p className="text-sm text-[#c8ccd8] leading-relaxed">
                                {userObj.bio || "This user hasn't added a bio yet." /* conditional for null user bio */}
                            </p>
                        </div>
                    </TabItem>
                </Tabs>
